...
infile_name = 'SILO_toogoolawah_post_office_WY2016-2021.csv'     # This input data used for calibartion.
...
C1 = float(42)           # Storage capacity [mm]
...
header1 = ['Date', 'Day', 'P', 'E', 'Qbase', 'Qsurf', 'Qtotal', 'Q', 'Q_Gregor']
header2 = ['[dd-mm-yyy]', '[-]', '[mm]', '[mm]', '[mm]', '[mm]', '[mm]', '[m^3/s]', '[m^3/s]']
# Using "zip" funciton to combine variables
outData = zip(Date, Day, P, E, Qbase, Qsurf, Qtotal, Q, Q_Gregor)
...
# stDate = datetime.datetime.strptime('2016-8-1', '%Y-%m-%d')
# enDate = datetime.datetime.strptime('2016-12-1', '%Y-%m-%d')

# # ===== Single plot example =====
# # fig, ax1 = plt.subplots() #Establishes figure handle
# fig, ax1 = plt.subplots(figsize=(12,9),dpi=100) #Establishes figure handles of size
# ax2 = ax1.twinx()
...
# fig.tight_layout()
# plt.savefig('AWBM - Model Output.png')

# plt.plot()


#%% Write calibration variables to file
import time
now = time.localtime()
current_time = time.strftime("%H:%M:%S", now)
    #variables in the following order: [date],A,A1,A2,A3,C1,C2,C3,R,NSE,RMSE,pbias
with open('gregors_calibration.txt', 'a') as the_file:
    the_file.write('\n' 
                   + str(current_time) + ','
                   + str(A) + ','
                   + str(A1) + ','
