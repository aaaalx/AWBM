...
from AWBMres_function import AWBMres_function 
...
infile_storage_curve = r"C:\Users\alex.xynias\OneDrive - Water Technology Pty Ltd\UQ\Thesis\Data\AWBM\wivenhoe_storage_curve.csv" # directory of csv with [Ha] and [ML]
...
for i_sim in range(0,nSims): # for each row in the batch input...
    ...
    headers_AWBM_results = ['Date'
                      ...
                      , 'Qbase', 'Qsurf', 'Qtotal', 'Q','resVolume[m3]'
                      ]
    ...
    data_day0 = [date_start
                 ...
                 ,float(),float() # BFR & SFR
                 ,BS_0, SS_0
                 ,float(),float(),float(),float(),float(),float(),float() # Qbase, Qsurf, Qtotal, Q, resVolume, resSpill[m3], resEvapLoss[m3]
                ]
...
# =============================================================================
    ...
    for i_day in range(0,nTS):        
        # Update reservoir storage's surface area.
            # This is so that storage loss to (climate change's impact on) evaporation can be accounted for.
            # Calculate an aproximate surface area of the storage volume
        A_res = storage_interp(infile_storage_curve,df_AWBM_results.loc[i_day,'resVolume[m3]'])
                #Takes a volume in [m^3] and returns the surface area [km^2] with 

        # AWBM_function(i_day,df_AWBM_results,df_data,C1,C2,C3,A1,A2,A3,BFI,BS_0,Kbase,SS_0,Ksurf,A,CN_P,CN_E,threshold_resVolume,outflow_human)
        AWBMres_function(i_day,df_AWBM_results,df_data,C1,C2,C3,A1,A2,A3,BFI,BS_0,Kbase,SS_0,Ksurf,A,CN_P,CN_E,threshold_resVolume,outflow_human,A_res)
...
# =============================================================================
    print('      Formating datetime column before output...')
    # Only the first timestep has the properly formatted datetime, so it needs to be filled out
        # it should be the same as the input data, so I'll just copy from that index    
    df_AWBM_results['Date'] = df_data[CN_time]
